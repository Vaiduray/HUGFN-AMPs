...
class SubTBTransformer(nn.Module):
    ...
    def model_params(self):
        return list(self.pos.parameters()) + list(self.embedding.parameters()) + list(self.encoder.parameters()) + \
            list(self.output.parameters()) + list(self.log_F_head.parameters())

    def Z_param(self):
        return self.Z_mod.parameters() if self.use_cond else [self.Z_mod]

    def forward(self, x, cond, mask, return_all=False, lens=None, logsoftmax=False, return_flow=False):
        x = self.embedding(x)
        x = self.pos(x)
        ...
        if return_all:
            ...
            if return_flow:
                log_F = self.log_F_head(final_rep).squeeze(-1)
                return self.logsoftmax2(out) if logsoftmax else out, log_F
