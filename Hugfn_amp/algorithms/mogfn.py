...
from Hugfn_amp.algorithms.base import BaseAlgorithm
from Hugfn_amp.algorithms.mogfn_utils.utils import mean_pairwise_distances, generate_simplex, thermometer, plot_pareto, pareto_frontier
from Hugfn_amp.algorithms.mogfn_subtb import smoothed_geometric_mean
from Hugfn_amp.utils import str_to_tokens, tokens_to_str
from Hugfn_amp.metrics import get_all_metrics
    ...
    def sample(self, episodes, cond_var=None, train=True):
        ...
        for t in (range(self.max_len) if episodes > 0 else []):
            ...
            if t <= self.min_len:
                ...
                logits[:, 0] += 10 # Force stopping after min length
